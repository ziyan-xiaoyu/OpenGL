...
{
    Point p0;
    if(button == GLUT_LEFT_BUTTON && action == GLUT_DOWN && iMode==4)//鼠标左击，绘制折线上的点
    {
        p0.x = xMouse;
    ...
    else if(button == GLUT_RIGHT_BUTTON && action == GLUT_DOWN && iMode==4)//鼠标右击，结束折线绘制
    {
        isEnd = TRUE;//结束
    }
}
...
int main(int argc, char* argv[])
{
    ...
    glutAddMenuEntry("Bezier", 4);//选择菜单4
