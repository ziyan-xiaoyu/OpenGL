...
struct Vec
{
  double x, y, z;    // 位置依旧满足color(R,G,B)
...
struct Ray        //定义光线结构Ray，包含起点o与方向d
{
    Vec o, d;
...
struct Sphere       //定义球体类型
{
  ...
  {
    Vec op = p-r.o;
  //将光线公式代入球体公式，解方程判断是否为实根且是否大于0
    double t, eps=1e-4, b=op.dot(r.d), det=b*b-op.dot(op)+rad*rad;
  //如果相交，返回交点与光线原点的距离，否则返回0
...
//人眼能够感知到的亮度，将颜色值转入0-255之间（使用的伽马值为2.2）
...
int main(int argc, char *argv[])
{
  ...
  int samps = argc==2 ? atoi(argv[1])/4 : 3; //采样率

//根据位置方向定义相机cam
  Ray cam(Vec(50,52,295.6), Vec(0,-0.042612,-1).norm());

//提前计算得到单个像素在相机x,y轴的偏移量cx,cy
  Vec cx=Vec(w*.5135/h), cy=(cx%cam.d).norm()*.5135;

//定义变量r存储计算后像素的颜色值，数组c存储各个像素值
  Vec r, *c=new Vec[w*h];

//对某一具体像素，将该像素划分为四个小方格，每个小方格中随机采样，samps条路径
    for (int y=0; y<h; y++)
    {
        ...
        for (unsigned short x=0, Xi[3]={0,0,y*y*y}; x<w; x++)
            for (int sy=0, i=(h-y-1)*w+x; sy<2; sy++)
                ...
                {
                //得到不同采样下不同路径后，调用radiance方法计算得到该小方格的最终亮度
...
//得到表示图像各像素的颜色数组c以后，写入文件（即将结果绘制在ppm文件中）
